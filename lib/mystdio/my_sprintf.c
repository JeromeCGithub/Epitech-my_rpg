...
**   None.
*/
static void my_sprintf_parse(char **res, char const *fstring,
...
** @EXAMPLE
**   my_sprintf(2, "Error: Couldn't parse '%s': file not found.", filename);
** @RETURN_VALUE
**   None.
*/
char *my_sprintf(char const *fstring, ...)
{
    ...
    for (unsigned int i = 0; fstring[i] != '\0'; i++) {
        if (fstring[i] == '%' && fstring[i + 1] != '\0') {
            i++;
            my_sprintf_parse(&response, fstring, &i, argp);
        } else {
            tmp[0] = fstring[i];
